<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
                        <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="交付类型" prop="jiaofuleixing">
          <el-input v-model="ruleForm.jiaofuleixing" 
              placeholder="交付类型" clearable  :readonly="ro.jiaofuleixing"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="交付类型" prop="jiaofuleixing">
              <el-input v-model="ruleForm.jiaofuleixing" 
                placeholder="交付类型" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
                              </el-row>
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `zujinjiaofu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.zujinjiaofuCrossAddOrUpdateFlag = false;
                  this.parent.search();
